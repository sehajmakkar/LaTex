...
export default function FreeAtsPage() {
  ...
  const handleAnalyze = async () => {
    setError(null);
    // The scan API needs an account until the anonymous check ships (Phase 4).
    if (!isSignedIn) {
      router.push("/sign-up?intent=ats");
      return;
    }
    if (!file) {
        ...
        <div className="mx-auto w-full max-w-4xl space-y-8">
          <section className="space-y-4 text-center">
            ...
            <p className="mx-auto max-w-xl text-sm text-muted-foreground">
              Upload your resume and get an ATS score, a summary and your top suggestions. Free
              accounts keep every report and can fix issues straight in the editor.
            </p>
          </section>
              ...
              <div className="flex flex-col items-stretch gap-2 sm:flex-row sm:items-center">
                ...
                >
                  {loading ? (
                    ...
                    "Create a free account to run the check"
                  ) : (
                    "Get free ATS score"
                  )}
                </Button>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex gap-2">
                <Button size="sm" asChild>
                  <Link href={isSignedIn ? "/ats" : "/sign-up?intent=ats"}>{isSignedIn ? "Open full ATS check" : "Create free account"}</Link>
                </Button>
                <Button size="sm" variant="outline" asChild>
                  <Link href={isSignedIn ? "/billing" : "/sign-up?intent=pro"}>View Pro plans</Link>
                </Button>
              </div>
            </div>
          </section>
